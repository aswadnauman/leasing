...
// Handle form submission for adding purchase returns
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_return'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO purchase_returns (return_id, purchase_id, supplier_id, outlet_id, return_date, total_amount, remarks, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssdss", $return_id, $purchase_id, $supplier_id, $outlet_id, $return_date, $total_amount, $remarks, $created_by);
        
        if ($stmt->execute()) {
            $success = "Purchase return added successfully!";
        } else {
            $error = "Error adding purchase return: " . $conn->error;
        }
        $stmt->close();
    }
}

// Fetch all purchase returns with related data
$purchase_returns_result = $conn->query("
    SELECT pr.*, p.purchase_id as original_purchase_id, s.supplier_name, o.outlet_name, u.username as created_by_name
    FROM purchase_returns pr
    JOIN purchases p ON pr.purchase_id = p.purchase_id
    JOIN suppliers s ON pr.supplier_id = s.supplier_id
    JOIN outlets o ON pr.outlet_id = o.outlet_id
    JOIN users u ON pr.created_by = u.user_id
    ORDER BY pr.created_at DESC
");

// Fetch purchases for dropdown
$purchases_result = $conn->query("SELECT purchase_id FROM purchases ORDER BY purchase_id");
...
<head>
    ...
    <title>Purchase Returns - Lease Management System</title>
...
<body>
    ...
    <div class="container mt-4">
        <div class="row">
            <div class="col-md-12">
                <h2>Purchase Returns Management</h2>
                    ...
                    <div class="card-body">
                        <form method="POST">
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        <label for="purchase_id" class="form-label">Original Purchase ID *</label>
                                        <select class="form-select" id="purchase_id" name="purchase_id" required>
                                            ...
                                            <?php if ($purchases_result && $purchases_result->num_rows > 0): ?>
                                                <?php while ($purchase = $purchases_result->fetch_assoc()): ?>
                                                    <option value="<?php echo $purchase['purchase_id']; ?>"><?php echo htmlspecialchars($purchase['purchase_id']); ?></option>
                                                <?php endwhile; ?>
                                            <?php endif; ?>
                                        </select>
                                    </div>
                            ...
                            <button type="submit" name="add_return" class="btn btn-primary">Add Purchase Return</button>
                        </form>
                    </div>
                </div>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tbody>
                                    <?php if ($purchase_returns_result && $purchase_returns_result->num_rows > 0): ?>
                                        <?php while ($return = $purchase_returns_result->fetch_assoc()): ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($return['return_id']); ?></td>
                                                <td><?php echo htmlspecialchars($return['original_purchase_id']); ?></td>
