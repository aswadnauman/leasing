...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_supplier'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO suppliers (supplier_id, supplier_name, contact_person, phone, email, address, city, outlet_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssss", $supplier_id, $supplier_name, $contact_person, $phone, $email, $address, $city, $outlet_id, $status);
        ...
        $stmt = $conn->prepare("UPDATE suppliers SET supplier_name=?, contact_person=?, phone=?, email=?, address=?, city=?, outlet_id=?, status=? WHERE id=?");
        $stmt->bind_param("ssssssssi", $supplier_name, $contact_person, $phone, $email, $address, $city, $outlet_id, $status, $id);
...
<head>
    ...
    <title>Suppliers - Lease Management System</title>
            ...
            <div class="col-md-12">
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <h5 id="form-title">Add New Supplier</h5>
                    </div>
                    <div class="card-body">
                        <form method="POST">
                            ...
                            <button type="submit" name="add_supplier" class="btn btn-primary" id="submit-btn">Add Supplier</button>
                                    ...
                                    <?php if ($suppliers_result && $suppliers_result->num_rows > 0): ?>
                                        <?php while ($supplier = $suppliers_result->fetch_assoc()): ?>
                                            <tr>
                                                ...
                                                <td><?php echo htmlspecialchars($supplier['outlet_name']); ?></td>
                                                <td>
                                                ...
                                                <td>
                                                    <button class="btn btn-sm btn-outline-primary edit-btn" 
                                                            ...
                                                            data-supplier_id="<?php echo htmlspecialchars($supplier['supplier_id']); ?>"
                                                            ...
                                                            data-city="<?php echo htmlspecialchars($supplier['city']); ?>"
                                                            data-outlet_id="<?php echo htmlspecialchars($supplier['outlet_id']); ?>"
                                                            data-status="<?php echo htmlspecialchars($supplier['status']); ?>">
                                                        Edit
                                                    </button>
    ...
    <script>
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('supplier_id').readOnly = true;
                ...
                document.querySelector('[name="add_supplier"]').name = 'update_supplier';
                document.querySelector('[name="update_supplier"]').textContent = 'Update Supplier';
        ...
        document.getElementById('cancel-btn').addEventListener('click', function() {
            ...
            document.getElementById('supplier_id').readOnly = false;
            ...
            document.querySelector('[name="update_supplier"]').name = 'add_supplier';
            document.querySelector('[name="add_supplier"]').textContent = 'Add Supplier';
